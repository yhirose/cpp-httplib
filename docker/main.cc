...
std::string log(const httplib::Request &req, const httplib::Response &res) {
    ...
    return std::format(
        R"({} - {} [{}] "{}" {} {} "{}" "{}")", req.remote_addr, remote_user, time_local(),
        request, res.status, body_bytes_sent, http_referer, http_user_agent);
}
...
int main(int argc, const char **argv) {
    ...
    svr.set_error_handler([](const httplib::Request & /*req*/, httplib::Response &res) {
        auto body = std::format(
            error_html, res.status, httplib::status_message(res.status), CPPHTTPLIB_VERSION);
